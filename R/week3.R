...
clean_df <- raw_df[raw_df$timeStart >= "2017-07-01 00:00:00 CET",]
clean_df <- clean_df[clean_df$q6 == 1,]

# Analysis
clean_df$timeSpent <- difftime(clean_df$timeEnd, clean_df$timeStart, tz = "CET", units = c("secs"))
hist(as.numeric(clean_df$timeSpent))
frequency_tables_list <- lapply(clean_df[,5:14], table)
frequency_tables_list <- lapply(frequency_tables_list, barplot)
sum(clean_df$q1 >= clean_df$q2 & clean_df$q2 != clean_df$q3)
frequency_tables_list
for(i in frequency_tables_list) {
  barplot(as.numeric(frequency_tables_list))
}